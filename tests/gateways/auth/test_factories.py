...
def describe_get_all_oauth_providers():
    ...
    def it_registers_gravatar_when_client_id_and_secret_are_both_configured(monkeypatch):
        gateways = get_settings().gateways
        monkeypatch.setattr(gateways, "google_oauth_client_id", None)
        monkeypatch.setattr(gateways, "gravatar_oauth_client_id", "some-client-id")
        monkeypatch.setattr(gateways, "gravatar_oauth_client_secret", SecretStr("some-client-secret"))

...
from canterlot.factories import ClubFactory, InviteTokenResponseFactory, UserFactory
...
def describe_create_club():
    ...
    def it_returns_422_for_a_name_that_is_too_short(client: TestClient, create_club_use_case: AsyncMock):
        response = client.post("/v1/clubs", json={"name": "ab"})

        assert response.status_code == 422
        create_club_use_case.execute.assert_not_called()
...
def describe_update_club_settings():
    def it_returns_200_with_the_updated_club(client: TestClient, club_service: AsyncMock):
    ...
    def it_returns_403_when_the_caller_lacks_privileges(client: TestClient, club_service: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()
    ...
    def it_returns_404_when_the_club_slug_does_not_exist(client: TestClient, club_service: AsyncMock):
    ...
    def it_returns_422_when_no_fields_are_provided(client: TestClient, club_service: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()
...
def describe_reject_pending_request():
    def it_returns_204_when_rejected(client: TestClient, club_service: AsyncMock, user_service: AsyncMock):
...
def describe_leave_club():
    def it_returns_204_when_left(client: TestClient, club_service: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()

        response = client.delete(f"/v1/clubs/{SOME_CLUB_SLUG}/members/me")

        assert response.status_code == 204
        club_service.leave_club.assert_awaited_once()
    ...
    def it_returns_409_when_the_caller_is_the_owner(client: TestClient, club_service: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()
    ...
    def it_returns_409_when_the_caller_is_a_protected_former_owner(client: TestClient, club_service: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()
...
def describe_remove_club_member():
    def it_returns_204_when_removed(
        ...
        assert response.status_code == 204
        remove_club_member_use_case.execute.assert_awaited_once()
    ...
    def it_returns_403_when_the_caller_lacks_sufficient_rank(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        remove_club_member_use_case.execute.side_effect = UnauthorizedClubMemberError("nope")
    ...
    def it_returns_404_when_the_target_is_not_a_member(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        remove_club_member_use_case.execute.side_effect = ClubMemberNotFoundError("not a member")
    ...
    def it_returns_409_when_the_target_is_a_protected_former_owner(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        remove_club_member_use_case.execute.side_effect = FormerOwnerProtectedError("protected")
...
def describe_change_club_member_role():
    def it_returns_204_when_changed(
        ...
        assert response.status_code == 204
        change_member_role_use_case.execute.assert_awaited_once()
    ...
    def it_returns_403_when_the_caller_is_not_owner(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        change_member_role_use_case.execute.side_effect = UnauthorizedClubMemberError("nope")
    ...
    def it_returns_404_when_the_target_is_not_a_member(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        change_member_role_use_case.execute.side_effect = ClubMemberNotFoundError("not a member")
    ...
    def it_returns_409_when_the_repository_reports_a_conflict(
        ...
        club_service.get_club_by_slug.return_value = _created_club()
        user_service.get_by_username.return_value = target_user
        change_member_role_use_case.execute.side_effect = MemberRoleChangeConflictError("stale")
...
def describe_create_ownership_transfer():
    def it_returns_201_with_the_reclaim_deadline_when_transferred(
        ...
        transfer_club_ownership_use_case.execute.return_value = OwnershipTransferResponse(reclaim_deadline=deadline)
...
def describe_dissolve_club():
    def it_returns_204_when_dissolved(client: TestClient, club_service: AsyncMock, dissolve_club_use_case: AsyncMock):
        club_service.get_club_by_slug.return_value = _created_club()

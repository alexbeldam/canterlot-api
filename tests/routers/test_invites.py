...
def describe_accept_invitation():
    ...
    def it_returns_400_for_an_invalid_invite_token(client: TestClient, accept_invite_use_case: AsyncMock):
        accept_invite_use_case.execute.side_effect = InvalidInviteTokenError("bad token")
    ...
    def it_returns_410_for_a_deactivated_invite(client: TestClient, accept_invite_use_case: AsyncMock):
        accept_invite_use_case.execute.side_effect = InviteLinkDeactivatedError("deactivated")

        response = client.patch(f"/v1/invites/{SOME_INVITE_ID}")

        assert response.status_code == 410

    def it_returns_403_for_a_direct_invite_identity_mismatch(client: TestClient, accept_invite_use_case: AsyncMock):
        accept_invite_use_case.execute.side_effect = DirectInviteIdentityMismatchError("mismatch")

        response = client.patch(f"/v1/invites/{SOME_INVITE_ID}")

        assert response.status_code == 403

    def it_returns_404_when_the_club_no_longer_exists(client: TestClient, accept_invite_use_case: AsyncMock):
        accept_invite_use_case.execute.side_effect = ClubNotFoundError("gone")

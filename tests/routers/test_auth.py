...
def describe_create_session():
    def it_logs_in_with_a_password_session(client: TestClient, create_session_use_case: AsyncMock):
        ...
        response = client.post(
            "/v1/auth/sessions",
            json={"type": "PASSWORD", "username": "alice_1", "password": "ValidP@ssword1"},
        )
    ...
    def it_returns_401_for_invalid_password_credentials(client: TestClient, create_session_use_case: AsyncMock):
        create_session_use_case.execute.side_effect = InvalidCredentialsError("nope")

        response = client.post(
            "/v1/auth/sessions",
            json={"type": "PASSWORD", "username": "alice_1", "password": "WrongP@ssword1"},
        )
    ...
    def it_returns_created_for_a_brand_new_oauth_account(client: TestClient, create_session_use_case: AsyncMock):
        create_session_use_case.execute.return_value = CreateSessionResult(
            access_token="access",
            refresh_token="refresh",
            is_new_user=True,
    ...
    def it_returns_401_for_an_invalid_oauth_credential(client: TestClient, create_session_use_case: AsyncMock):
        create_session_use_case.execute.side_effect = InvalidOAuthCredentialError("bad token")
    ...
    def it_returns_422_for_an_unrecognized_provider(client: TestClient, create_session_use_case: AsyncMock):
        response = client.post(
            "/v1/auth/sessions",
...
def describe_login_swagger_shim():
    ...
    def it_still_logs_in_via_the_form_encoded_oauth2_password_flow(client: TestClient, auth_service: AsyncMock):
        ...
        auth_service.login_user.assert_awaited_once_with(
            username="alice_1",
            plain_password=SecretStr("secret1"),
        )
...
def describe_rotate_session():
    def it_returns_a_rotated_access_token_and_sets_a_new_refresh_cookie(client: TestClient, auth_service: AsyncMock):
        auth_service.rotate_refresh_token.return_value = TokenResponseFactory.build(

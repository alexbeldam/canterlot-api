...
def _decode_dict(raw_dict: dict[Any, Any]) -> dict[str, str]:
    return {
...
class RedisRepository(CacheRepository, DatabaseRepository, RateLimiter):
    ...
    async def find(self, key: str) -> dict[str, str] | None:
        ...
        return _decode_dict(raw_dict)

    async def find_many(self, keys: list[str]) -> list[dict[str, str] | None]:
        if not keys:
            return []

        async with self.__redis.pipeline(transaction=False) as pipe:
            for key in keys:
                pipe.hgetall(key)
            raw_dicts = await pipe.execute()

        return [_decode_dict(raw) if raw else None for raw in raw_dicts]
